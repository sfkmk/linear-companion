...
describe('linear', () => {
  let parseLinearTitle: any;
  let extractIssueId: any;

  beforeAll(async () => {
    // Use dynamic import after mock registration to ensure @raycast/utils is mocked
    // before the linear module is evaluated.
    const mod = await import('./linear');
    parseLinearTitle = mod.parseLinearTitle;
    extractIssueId = mod.extractIssueId;
  });
