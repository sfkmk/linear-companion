...
export default function Command() {
  ...
  const isBacklinkingEnabled = Boolean(preferences.backlinkUrlScheme?.trim());
  ...
  const canProcess = isAuth || !isBacklinkingEnabled;
  ...
  async function startProcessing() {
    if (!isAuth && isBacklinkingEnabled) {
      await showToast({
        style: Toast.Style.Failure,
        title: 'Authentication Required',
        message: 'Please connect Linear first.',
      });
      return;
    }
    ...
    if (!isAuth && !isBacklinkingEnabled) {
      await showToast({
        ...
        title: 'Backlinking disabled',
        message: 'Processing will mark items as disabled without requiring authentication.',
      });
    }
  ...
  return (
    <List
      ...
      navigationTitle={
        isProcessing ? `Processing ${progress.current}/${progress.total}` : `Found ${items.length} Folders`
      }
      ...
      {items.length > 0 ? (
        <>
          {!isProcessing && progress.current === 0 && (
            <List.Item
              title={canProcess ? 'Start Backlinking' : 'Connect Linear to Start'}
              icon={canProcess ? Icon.Play : Icon.Key}
              actions={
                <ActionPanel>
                  {canProcess ? (
            ...
            {items.map((item) => (
              <List.Item
                ...
                actions={
                  !isProcessing && (
                    <ActionPanel>
                      {!canProcess && <Action title="Connect Linear" onAction={handleAuthorize} />}
                      {canProcess && (
                        <Action
